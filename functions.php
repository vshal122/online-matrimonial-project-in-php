...
function register(){
	...
	$sql = "INSERT 
			INTO
			   users
			   ( profilestat, username, password, email, dateofbirth, gender,mobile,userlevel) 
			VALUES
			   (0, '$uname', '$pass', '$email', '$dob', '$gender','$mbl',0)";

	if (mysqli_query($conn,$sql)) {
	  $msg="Registration Successfull Now you can log in your Account";
            header("location:http://localhost/online-matrimonial-project-in-php-master/login.php?msg=".$msg);
            exit();
    } else {
...
$sql="SELECT id FROM documents WHERE cust_id = '$id'";
$result = mysqlexec($sql);

//code part to check weather a photo already exists
if(mysqli_num_rows($result) == 0) {
     // no photo for curret user, do stuff...
		$sql="INSERT INTO documents (id, cust_id, pic1, pic2, pic3, pic4) VALUES ('', '$id', '$pic1' ,'$pic2', '$pic3','$pic4')";
		// Writes the information to the database
		mysqlexec($sql);
    ...
    // There is a photo for customer so up
     $sql="UPDATE documents SET pic1 = '$pic1', pic2 = '$pic2', pic3 = '$pic3', pic4 = '$pic4' WHERE cust_id=$id";
		// Writes the information to the database
	mysqlexec($sql);
}
